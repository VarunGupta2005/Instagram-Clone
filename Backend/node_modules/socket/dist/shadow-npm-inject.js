...
function findBestPatchVersion(node, availableVersions, vulnerableVersionRange) {
...
function findPackageNodes(tree, name, version) {
  ...
  while (queue.length) {
    ...
    if (!targetVersion && Array.isArray(node.errors) && node.errors.length) {
      debug.debugFn(`miss: version for ${node.name} due to errors:\n`, node.errors);
    }
...
function updatePackageJsonFromNode(editablePkgJson, tree, node, newVersion, rangeStyle) {
  ...
  for (const depField of ['dependencies', 'optionalDependencies', 'peerDependencies']) {
    ...
    const depValue = objects.hasOwn(depObject, name) ? depObject[name] : undefined;
...
//# debugId=22ee222a-71cf-4fc6-ad06-f9ae02287129
